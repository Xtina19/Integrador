...
import type { MovimientoFiltroId, MovimientoTipoUi, MovimientoVista } from '../types/inventoryUi'
import { MOVIMIENTO_TIPO_LABEL } from '../types/inventoryUi'
import { MOVIMIENTO_CONTEXT } from '../utils/movimientosContext'

const tipoGroups: {
  id: Exclude<MovimientoFiltroId, 'all'>
  label: string
...
interface Props {
  ...
  onFiltroChange: (filtro: MovimientoFiltroId) => void
...
export function MovimientosTab({
  ...
  const filtered = useMemo(() => {
    const grupo = tipoGroups.find((g) => g.id === filtro)
    return movimientos.filter((m) => {
      const q = search.toLowerCase()
      const matchSearch = !q || m.productoTitulo.toLowerCase().includes(q) || m.id.toLowerCase().includes(q)
      const matchGrupo =
        !grupo ||
        (grupo.tipos ? grupo.tipos.includes(m.tipo) : false) ||
        (grupo.documentoTipos ? grupo.documentoTipos.includes(m.documentoTipo) : false)
      ...
      return matchSearch && (filtro === 'all' || matchGrupo) && matchSuc && matchUser && matchDoc
    })
  }, [movimientos, search, filtro, sucursal, usuario, documento])

  const context = MOVIMIENTO_CONTEXT[filtro]

  return (
    <Card>
      <CardHeader title={context.title} subtitle={context.description} />
      <CardBody className="!p-0">
        <div className="space-y-4 p-4 pb-0">
          <div className="flex flex-wrap gap-2">
            <Chip active={filtro === 'all'} onClick={() => onFiltroChange('all')} label="Todos" />
            {tipoGroups.map((g) => (
              <Chip
                key={g.id}
                active={filtro === g.id}
                onClick={() => onFiltroChange(g.id)}
                label={g.label}
              />
            ))}
          </div>
