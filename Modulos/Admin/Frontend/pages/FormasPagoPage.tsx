...
import { formasPagoApi } from '@/services/api/formasPagoApi'

type Row = { id: string; code: string; name: string; status: string; description?: string }
...
export function FormasPagoPage() {
  ...
  const load = useCallback(async () => {
    try {
      const list = await formasPagoApi.list()
      setRows(list as Row[])
    } catch (err) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (dialog?.mode === 'create') {
        await formasPagoApi.create(body)
        showSuccess('Registro creado')
      } else if (dialog?.id) {
        await formasPagoApi.update(dialog.id, body)
  ...
  async function toggle(r: Row) {
    try {
      await formasPagoApi.setEstado(r.id, r.status === 'active' ? 'inactive' : 'active')
  ...
  return (
    <div className="space-y-6">
      ...
      <FormDialog
        ...
        title={dialog?.mode === 'create' ? 'Nueva Formas de Pago' : dialog?.mode === 'edit' ? 'Editar Formas de Pago' : 'Detalle'}
