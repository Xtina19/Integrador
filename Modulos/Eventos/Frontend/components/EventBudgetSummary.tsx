  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-surface p-6 space-y-4 max-w-md">
      <div className="flex justify-between items-baseline">
        ...
        <span className="text-xl font-bold text-corporate tabular-nums">{formatDop(summary.budget)}</span>
      </div>
      <div className="border-t border-gray-200 pt-4 space-y-3">
        <div className="flex justify-between text-sm">
          <span className="text-gray-600">Costo utensilios</span>
          <span className="font-medium tabular-nums">{formatDop(summary.utensilsCost)}</span>
        </div>
        <div className="flex justify-between text-sm items-center gap-4">
          <span className="text-gray-600">Costo operativo</span>
          {readOnly || !onOperationalCostChange ? (
            <span className="font-medium tabular-nums">{formatDop(summary.operationalCost)}</span>
          ) : (
            <input
              type="number"
              min={0}
        ...
        <div className="flex justify-between text-sm font-semibold pt-2 border-t border-gray-100">
          ...
          <span className="text-corporate tabular-nums">{formatDop(summary.totalSpent)}</span>
        </div>
        <div className="flex justify-between text-sm font-semibold">
          <span className="text-gray-800">Disponible</span>
          <span className={`tabular-nums ${summary.available >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
            {formatDop(summary.available)}
          </span>
        </div>
      </div>
    </div>
