...
import { SaleRecordDialog } from '@/modules/ventas/components/SaleRecordDialog'
...
type DialogState = { saleId: string; mode: 'view' | 'exchange' } | null
...
export function HistorialVentasPage() {
  ...
  const selectedSale = dialog ? salesHistory.find((s) => s.id === dialog.saleId) ?? null : null
  ...
  return (
    <div className="space-y-6">
          ...
          <Table
            ...
            columns={[
              ...
              },
              {
                ...
                render: (s) => (
                  <TableActions
                    onView={() => setDialog({ saleId: s.id, mode: 'view' })}
                    onExchange={
                      s.status === 'paid'
                        ? () => setDialog({ saleId: s.id, mode: 'exchange' })
                        : undefined
                    }
                  />
                ),
      ...
      <SaleRecordDialog
        sale={selectedSale}
        open={dialog?.mode === 'view' && Boolean(selectedSale)}
        onClose={() => setDialog(null)}
      />

      <SaleExchangeDialog
        open={dialog?.mode === 'exchange' && Boolean(selectedSale)}
        onClose={() => setDialog(null)}
