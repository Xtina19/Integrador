...
import { notificationEmails } from '@/mocks/mockConfiguracion'
import { useTableExport } from '@/hooks/useTableExport'

export function CorreosPage() {
  const { onExportPdf, onExportExcel } = useTableExport('Correos')
      ...
      <Card>
        <CardBody>
          <Toolbar
            ...
            onExportExcel={() =>
              onExportExcel(
                ['ID', 'Evento', 'Destinatarios', 'Estado'],
                filtered.map((e) => [e.id, e.event, e.recipients, e.active ? 'Activo' : 'Inactivo'])
              )
            }
          />
        </CardBody>
